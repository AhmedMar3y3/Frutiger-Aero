...
const worlds = [
  ...
  { name: 'Night', icon: Moon, color: 'night' },
];

// Keep the animated scenery out of the music player's time-update renders.
const SceneLife = memo(function SceneLife() {
  return <div className="life" aria-hidden="true">
    ...
    {Array.from({ length: 5 }, (_, i) => <div className={`swimmer swimmer-${i}`} key={i}><img src="/goldfish.webp" width="600" height="400" decoding="async" alt="" draggable="false" /></div>)}
    <div className="glass-orb orb-one" /><div className="glass-orb orb-two" />
    <div className="water-motes">{Array.from({ length: 8 }, (_, i) => <span key={i} style={{ left: `${9 + i * 12}%`, top: `${35 + (i * 17 % 53)}%`, animationDelay: `${-i * 1.7}s` }} />)}</div>
  </div>;
});
...
export default function Home() {
  ...
  const userChoseAudio = useRef(false);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onStarted = () => { removeUnlockListeners(); if (!disposed) setAutoplayBlocked(false); };
    function unlockAudio(event: Event) {
      if (userChoseAudio.current || disposed) { removeUnlockListeners(); return; }
      if (event.target instanceof Element && event.target.closest('[data-audio-control]')) return;
      void player!.play().then(onStarted).catch(() => { /* Retry on another user gesture. */ });
    }
    document.addEventListener('pointerdown', unlockAudio);
    document.addEventListener('keydown', unlockAudio);
    player.addEventListener('play', onStarted);
    void player.play().then(onStarted).catch(error => {
      if (!disposed && error.name === 'NotAllowedError') setAutoplayBlocked(true);
    });
    return () => { disposed = true; removeUnlockListeners(); player.removeEventListener('play', onStarted); };
  }, []);
  useEffect(() => {
    const scene = stage.current;
    const updateVisibility = () => { if (scene) scene.dataset.paused = String(document.hidden); };
    updateVisibility();
  ...
  return (
    <div ref={stage} className={`aero-world world-${world} ${motion ? '' : 'still'} ${immersed ? 'immersed' : ''}`}>
      <audio ref={audio} src="/new-look.mp3" autoPlay loop preload="auto" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onTimeUpdate={event => setElapsed(Math.floor(event.currentTarget.currentTime))} onError={() => { setPlaying(false); setAudioError('The music could not load. Please refresh and try again.'); }} />
      ...
      <header className="site-header chrome">
        ...
        <button data-audio-control className={`sound-toggle glass ${playing ? 'is-playing' : ''}`} onClick={toggleAudio}><AudioLines size={17} /><span>{autoplayBlocked ? 'Enable sound' : `Sound ${playing ? 'on' : 'off'}`}</span><span className="status-dot" /></button>
      </header>
      <main id="experience">
        <div className="hero chrome">
          <div className="eyebrow"><span /> YOUR INTERNET ESCAPE, CIRCA 2007</div>
          <h1>The future<br />we <span>dreamed of.</span></h1>
          <p>{world === 1 ? <>Beneath the surface, everything slows down.<br className="desktop-break" /> Let the light drift. Let your mind wander.</> : world === 2 ? <>City lights. A quieter world. Just you,<br className="desktop-break" /> the moon, and a little late-night daydream.</> : <>Clear skies. A quieter mind. A world that feels<br className="desktop-break" /> like a memory you haven’t made yet.</>}</p>
        ...
        <div className="world-marker chrome"><span className="tiny-cross">+</span><div>{world === 1 ? 'A LITTLE WORLD BELOW THE SURFACE' : 'YOU ARE SOMEWHERE BETTER'}<span>{world === 2 ? '02:07 AM · UNDER THE SAME MOON' : world === 1 ? 'CRYSTAL CLEAR · ENDLESSLY DRIFTING' : '25° · ALWAYS A LITTLE SUNNY'}</span></div></div>
        ...
        <section className="bottom-deck chrome" aria-label="Experience controls">
          ...
          <div className="player glass">
            <div className="player-main"><div className="album-art"><span><Waves size={27} /></span></div><div className="track-info"><div className="section-label">AERO RADIO <span className="radio-live">● LOFI</span></div><h2>New Look</h2><p>Wii U Mii Maker · Lofi Mix</p></div><button data-audio-control className="play-button" onClick={toggleAudio} aria-label={playing ? 'Pause music' : 'Play music'}>{playing ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" />}</button></div>
            ...
            {autoplayBlocked && !audioError && <p className="autoplay-note">Tap anywhere to let the music in.</p>}
          </div>
        </section>
      </main>
